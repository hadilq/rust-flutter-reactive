...
pub struct MainPageReducerStruct<SuccessorType: Successor> {
    pub successor: SuccessorType,
}

impl<SuccessorType: Successor> MainPageReducer for MainPageReducerStruct<SuccessorType> {
    fn reduce(
        self: &Self,
        state: &MainState,
        action: &MainAction,
        ...
        let count = match action {
            MainAction::None => state.text.count,
            MainAction::ButtonClicked => self.successor.add_one(state.text.count),
        };
        MainState {
            text: Text { count },
        }
